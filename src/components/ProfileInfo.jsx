    ...
    <div className="user_info_container">
     {errors && <div className="errors">{errors.map((err, i) => (<div key={i}>{err}</div>))}</div>}
        <div className="left_side">
          {myProfile &&  <div className="setting_btn"> <SettingsIcon  className="gear" onClick={() => { setUpdateing(!updateing)}}/></div>}
          
          {/* IF User is updating settings */}
          {updateing && myProfile && <>
            <div className="flex privacy_btn_wrapper " > Public   
            <div onClick={ () => {
            ...
            <div className="profile_image_container">
              ...
              <label  className="flex" id="edit_icon" htmlFor="avatar"><EditIcon/></label>
              <input  id="avatar" type="file"   accept="image/*,.png, .jpg, .jpeg, .gif" onChange={()=>{convertImg(document.getElementById("avatar").files).then(res => setImg(res))}} />
            ...
            <Input className="update_field" type={"text"} defaultValue={updateInfo.nickname} onInput={(e) => {updateInfo.nickname = e.target.value}}>{data.nickname}</Input>
            <Button onClick={()=>{handleUpdate(updateInfo)}}>Update</Button>
          </>}

          {!updateing && <>
            <Typography variant="h6" gutterBottom> Profile is {data.is_private ? "private" : "public"} </Typography>
            <Avatar  sx={{ width: 70,height: 70,}} alt={data.first_name} src={data.user_img} />
